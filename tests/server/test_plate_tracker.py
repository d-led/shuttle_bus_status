...
from camera.config import DebouncingSettings, LoggingSettings
...
class TestPlateTracker:
    ...
    def test_arrival_after_threshold(self) -> None:
        ...
        assert tracker._tracker._plates["ABC123"].arrived_at == now + timedelta(
            seconds=1.0
        )
    ...
    def test_no_logging(self) -> None:
        ...
        now = datetime.now(UTC)
        with patch.object(tracker, "_log_file"):
            tracker.update(["ABC123"], now)
            tracker.close()
